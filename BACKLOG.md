...
- [ ] **Onglet/section Fees du PoolDetail avec *graphes*** (barème configuré + frais effectif dans le temps) — blocker data **levé** (la CA agrège désormais les fees, migration 017), reste à faire : (1) un **endpoint d'historique fee time-series** — les fees ne sont exposés qu'en **scalaire 24h** sur `/api/pools`, pas en série par bucket ; lire les buckets de la CA (`liquidity`/`claim_*` CA ont aussi des read-paths à brancher, cf. Transverse) ; (2) les **graphes** eux-mêmes. À ouvrir quand le cadrage produit de l'onglet Fees est acté
