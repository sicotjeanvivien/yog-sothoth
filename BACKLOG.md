...
2. [x] **`GET /api/signals/stream`** — SSE alimenté par le poller interne. Livré (2 juil.) :
	...
	4. [x] **vérif live** : 328 signaux générés par le signal-engine pendant le stream ouvert → tous reçus en direct (`data:` + `id:` + ping keep-alive) ; reconnexion → 0 replay ; CORS OK sur `text/event-stream` (ACAO echo de l'origine autorisée) ; table nettoyée
3. [x] **Page feed dans le dashboard** — la partie visible de la boucle. **Plus de BFF** → `EventSource` direct sur `/api/signals/stream`, zéro proxy. NB : `CLAUDE.md` + `web/README.md` décrivent encore l'archi BFF → dérive de doc à corriger (audit ou au passage). Livré (2 juil.) :
...
- [ ] **Split ISP des traits repository mixtes** (relevé revue PR #38, mécanique) : appliquer le pattern `PoolAccountResolver`/`SignalFeedRepository` — une lunette par consommateur, même struct `Pg` — aux traits qui mélangent encore write-side et read-side avec des consommateurs disjoints : `TokenPriceRepository` (context écrit / api lit), `MeteoraDammV2SwapEventRepository` + `MeteoraDammV2LiquidityEventRepository` (indexer écrit / api lit), `PoolRepository` lui-même (sections write indexer / read api dans un seul trait). Gain : mocks sans `unreachable!()`, dépendances exactes par binaire ; le garde-fou runtime reste le rôle Postgres
