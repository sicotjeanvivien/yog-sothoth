...
- [x] **Déploiement de l'`InfoPopover`** — **livré 6 juil. 2026** : 4 KPIs Overview + 4 cards pool-detail (TVL/volume/fees/composition) + 5 lignes analytiques (fee tier, fee split, fee effectif, parts protocole/LP), textes en/fr vérifiés contre la sémantique réelle (`PoolAnalytics`, règles de nullité). Prop `info?` optionnelle sur `StatCard`/`KpiCard`/`InfoRow`/`PoolCompositionCard`, clé générique `shell.metricInfo`. Détecteurs sur `/signals` livrés dans la foulée : un ⓘ unique sur le groupe de filtres « Type » (une place, pas 50 répétitions par card), listant les détecteurs connus présents ; aucun chiffre de seuil en dur dans les textes (config env — chaque card affiche le seuil réellement franchi).
- [ ] **PoolDetail : onglet « Alertes »** (relevé 6 juil. 2026) — 5ᵉ onglet (après Info/Swaps/Liquidité/Fees) listant les signaux émis pour la pool, rendus avec la `SignalCard` existante. ⚠️ Nécessite un support API : `GET /api/signals` ne filtre que par sévérité aujourd'hui — ajouter un filtre `?pool=` (lens `SignalFeed`, un paramètre d'égalité optionnel dans le SQL statique, même pattern que `severity`) + le paramètre dans le handler et le fetcher web. Live SSE optionnel en v1 (filtrage client du flux global par adresse, la machinerie existe).
