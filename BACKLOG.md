...
#### ✅ Communication utilisateurs — annonces + changelog (cadré 9 juil. 2026, livré 20 juil. 2026)
...
- [x] **Annonces opérateur — livré 20 juil. 2026** : table `announcements` (migration 026, table normale — pas hypertable ; CHECKs kind/severity/fenêtre ; GRANT SELECT `yog_api` seul, publication INSERT psql admin comme prévu) + `GET /api/announcements/active` (non paginé, cap dur 10, tri sévérité puis récence en SQL) + bandeau dismissible dans le layout dashboard (fetch + pick **server-side**, pattern cookie sidebar → zéro flash ; **un seul cookie CSV** d'ids dismissés, plafonné 20 ids / 90 j, plutôt qu'un cookie par annonce ; un bandeau à la fois — le plus sévère). Décisions actées au cadrage : **message anglais seul** (i18n du chrome uniquement ; escalade documentée `message_fr` nullable si demande), **sévérité `AnnouncementSeverity` distincte** du Signal Engine (choix éditorial ≠ conclusion détecteur — seuls les tokens couleur du design system sont partagés), bandeau dashboard uniquement, échec du fetch → pas de bandeau (jamais le dashboard à terre). Vérifié e2e : fenêtre (expirée/future exclues), tri, SSR en/fr, skip du cookie côté serveur. Modale critique différée au premier vrai cas, inchangé
- [x] **Page `/changelog` — livrée 20 juil. 2026** : statique dans le web, **sans MDX** (une grappe de deps pour une page — rejeté) : fichier de données typé `releases.ts` (pattern pur-data du footer, garde-fous vitest : versions uniques `vX.Y.Z`, tri desc, sections non vides) + composants Server purs sur le template terms/privacy. Un bloc ancré par release (`id` = version, `scroll-mt` sous la navbar) — cible du `link_url` des annonces release. Contenu anglais seul (décision partagée avec les annonces), chrome i18n en/fr (`changelog.json`), lien footer colonne Produit. Vérifié e2e : SSR en/fr, et le flux complet bandeau release → « Read more » → atterrissage sur l'ancre `#v0.1.1` au navigateur piloté. Blocs initiaux v0.1.1 + v0.1.0
...
- [x] **Frontend / PagePool** : système de favoris stocké en LocalStorage (fait, watchlist) — étoile sur la PagePool + page `/watchlist` (nav dédiée) alimentée par N `fetchPool` client-side (front pur, pas d'endpoint batch). Hook `useWatchlist` sur `useSyncExternalStore` (SSR-safe, synchro inter-onglets) → réutilisable tel quel pour la watchlist serveur v0.3.
...
### Étude comparative RPC — résultats (20 juillet 2026, révisée le 20 juillet après ajout d'Alchemy)
...
**Budget réel** : Alchemy/Triton en PAYG peuvent démarrer bien en dessous de 150 $ (facturé/plafonné à la conso réelle, faible en phase de dev) ; sinon plancher flat ~150–200 $/mois (Shyft) jusqu'à ~500 $/mois (Helius, QuickNode) — de toute façon bien au-dessus du `~20 € HT` provisoire posé le 3 juillet.

**Recommandation révisée (sous réserve de devis directs) — priorité actée le 20 juillet : coût plafonné et maîtrisé pour la phase de dev/migration, pas un abonnement récurrent indépendant de la conso.**

1. **Alchemy PAYG en tête** — même logique « paye ce que tu consommes » que Triton, mais mécanisme de plafonnement (« Usage Limit ») **documenté** plutôt que supposé, pas de dépôt minimum non remboursable perdu si la conso réelle reste faible en dev, et légèrement moins cher au To (75 $ vs ≈82 $).
2. **Triton PAYG en second** — le mécanisme de solde zéro reste un inconnu réel malgré recherche approfondie ; à ne retenir qu'après confirmation directe de leur équipe sur le comportement de coupure. Le dépôt de 125 $ est non remboursable même si la conso réelle est faible — désavantage face à Alchemy pour une phase de dev à faible volume.
3. **Shyft Build (199 $/mo flat)** reste le choix si on préfère la prévisibilité totale d'un forfait plutôt qu'un PAYG, une fois le volume réel connu (post-mesure Alchemy/Triton).
