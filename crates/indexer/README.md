...
`tests/geyser_server.rs` serves a test-written script over loopback, and every ending
of a stream has a test that goes red when it changes its answer to which ending
restarts the retry budget, which charges it, and what the next attempt asks for
— the budget being `run`'s `match`, and the resume point the one expression
beside it, `Attempt::next_resume_from`. The backoff reset is the one decision left unguarded on purpose — its
