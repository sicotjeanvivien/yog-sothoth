...
impl Config {
    pub(crate) fn load() -> Result<Self, ConfigError> {
        // `INGEST_SOURCE` is read and validated, but not carried on `Config`:
        // with a single implemented source there is nothing downstream to
        // hand it to, and a field nobody reads is a field nobody maintains.
        // It joins the struct the day `GrpcListener` gives it a reader.
