...
│   └── pool_account.rs    (two-level PoolAccountProperties + PoolAccountResolver)
├── application/
...
│   └── pool_account/      ← account bytes → pool properties use case
...
- **Pool-account decoding** (`application/pool_account/`) — the counterpart of the above: turns an on-chain account's raw bytes into `PoolAccountProperties`, the properties events never carry (mints, base fee, fee split). Dispatches on the account's `owner` — which on Solana *is* its program id — via `Protocol::from_program_id`, so callers need not know which protocol they asked for and one RPC client can serve them all. Two guards on every decode, neither redundant: the owner (by dispatch) and the account's Anchor discriminator. They are not defensive — at cp-amm's mint offsets a DLMM `LbPair` holds `reserve_x`/`reserve_y`, valid aligned `Pubkey`s, so an unguarded decode succeeds and writes vault addresses into mint columns.
