//! Unit tests for `TryFrom<MeteoraDammV2ClaimRewardEventRow> for MeteoraDammV2ClaimRewardEvent`.
//!
...
use super::MeteoraDammV2ClaimRewardEventRow;
...
fn valid_row() -> MeteoraDammV2ClaimRewardEventRow {
...
fn try_from_valid_row_returns_event_with_all_fields_mapped() {
    let event =
        MeteoraDammV2ClaimRewardEvent::try_from(valid_row()).expect("valid row should convert");
...
fn try_from_preserves_signature_and_timestamp() {
    ...
    let row = MeteoraDammV2ClaimRewardEventRow {
        signature: expected_sig.to_string(),
        timestamp,
        ..valid_row()
    };

    let event = MeteoraDammV2ClaimRewardEvent::try_from(row).expect("valid row should convert");
...
fn try_from_invalid_pool_address_returns_integrity() {
    ...
    let err = MeteoraDammV2ClaimRewardEvent::try_from(row).expect_err("invalid pubkey should fail");
    assert!(
...
fn try_from_invalid_position_returns_integrity() {
    ...
    let err =
        MeteoraDammV2ClaimRewardEvent::try_from(row).expect_err("invalid position should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_owner_returns_integrity() {
    ...
    let err = MeteoraDammV2ClaimRewardEvent::try_from(row).expect_err("invalid owner should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_mint_reward_returns_integrity() {
    ...
    let err =
        MeteoraDammV2ClaimRewardEvent::try_from(row).expect_err("invalid mint_reward should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_signature_returns_integrity() {
    ...
    let err =
        MeteoraDammV2ClaimRewardEvent::try_from(row).expect_err("invalid signature should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_negative_total_reward_returns_integrity() {
    ...
    let err = MeteoraDammV2ClaimRewardEvent::try_from(row)
