//! Unit tests for `TryFrom<ClaimPositionFeeEventRow> for ClaimPositionFeeEvent`.
//!
...
use super::ClaimPositionFeeEventRow;
...
fn valid_row() -> ClaimPositionFeeEventRow {
...
fn try_from_valid_row_returns_event_with_all_fields_mapped() {
    let event = ClaimPositionFeeEvent::try_from(valid_row()).expect("valid row should convert");
...
fn try_from_preserves_signature_and_timestamp() {
    ...
    let row = ClaimPositionFeeEventRow {
        signature: expected_sig.to_string(),
        timestamp,
        ..valid_row()
    };

    let event = ClaimPositionFeeEvent::try_from(row).expect("valid row should convert");
...
fn try_from_invalid_pool_address_returns_integrity() {
    ...
    let err = ClaimPositionFeeEvent::try_from(row).expect_err("invalid pubkey should fail");
    assert!(
...
fn try_from_invalid_position_returns_integrity() {
    ...
    let err = ClaimPositionFeeEvent::try_from(row).expect_err("invalid position should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_owner_returns_integrity() {
    ...
    let err = ClaimPositionFeeEvent::try_from(row).expect_err("invalid owner should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_signature_returns_integrity() {
    ...
    let err = ClaimPositionFeeEvent::try_from(row).expect_err("invalid signature should fail");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
...
fn try_from_invalid_protocol_returns_integrity_with_message() {
    ...
    let err = ClaimPositionFeeEvent::try_from(row).expect_err("unknown protocol should fail");
    let msg = match err {
...
fn try_from_negative_fee_a_claimed_returns_integrity() {
    ...
    let err =
        ClaimPositionFeeEvent::try_from(row).expect_err("negative i64 should fail u64 conversion");
    assert!(
        matches!(err, RepositoryError::Integrity(_)),
        "expected Integrity, got {err:?}"
    );
