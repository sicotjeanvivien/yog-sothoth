...
- **[`bootstrap` (`yog-bootstrap`)](./bootstrap/README.md)** — shared startup utilities: env parsing primitives, the two secret types (`SecretUrl`, `SecretKey`) and the `Endpoint` that holds an address apart from its credential, `ConfigError`, `init_rustls()`, `init_tracing()`, and the shared stop (`shutdown_signal()`, `TaskEnd`, `Stop`). The decision rule for adding anything: *does this run identically in every binary's `main()`?* If it varies even slightly, it stays in the binary.
