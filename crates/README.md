...
- **[`indexer` (`yog-indexer`)](./indexer/README.md)** — the ingest daemon. A `TransactionSource` port (the JSON-RPC source owning its listener → dispatcher → fetch stages) feeding a bounded worker, `TransactionProcessor`, per-protocol sub-persistors, Prometheus metrics.
