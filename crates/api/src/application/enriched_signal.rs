...
/// `Clone`: the poller enriches a signal once and shares it with every
/// stream behind an `Arc`; each stream clones it to build its response.
#[derive(Debug, Clone)]
...
impl EnrichedSignal {
    ...
    /// unresolved. Used by the SSE poller when enrichment fails —
