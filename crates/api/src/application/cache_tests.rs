...
async fn concurrent_callers_of_a_cold_key_share_one_computation() {
    let cache = Arc::new(TtlCache::<(), u32, String>::new(Duration::from_secs(30)));
...
async fn an_expired_value_is_computed_again() {
    let cache = TtlCache::<(), u32, String>::new(Duration::from_secs(30));
...
async fn each_key_has_its_own_value() {
    let cache = TtlCache::<u8, u32, String>::new(Duration::from_secs(30));
...
/// every caller answered at once — instead of each redoing it in turn.
///
/// Mutation: retry inside the flight (`get_or_try_init`), and the count is
/// the number of callers.
#[tokio::test]
async fn concurrent_callers_of_a_failing_computation_share_its_error() {
    let cache = Arc::new(TtlCache::<(), u32, String>::new(Duration::from_secs(30)));
    let calls = Arc::new(AtomicUsize::new(0));
    ...
    assert!(
        answers
            .into_iter()
            .all(|a| a.unwrap() == Err("no slot".to_string()))
    );
    assert_eq!(calls.load(Ordering::SeqCst), 1);
}
